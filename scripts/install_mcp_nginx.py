#!/usr/bin/env python3
"""Idempotently attach the FeatureRequest MCP routing include to its HTTPS server."""
...
BEGIN = "    # BEGIN FEATURE REQUEST MCP DEPLOY CONTRACT"
END = "    # END FEATURE REQUEST MCP DEPLOY CONTRACT"
...
def render_site(text: str, include_path: str) -> str:
    if BEGIN in text or END in text:
        ...
        return text
    start, end = _https_server_bounds(text)
    block = text[start:end]
    anchor = "    client_max_body_size 20M;"
    position = block.find(anchor)
    if position < 0:
        raise ValueError("HTTPS server block lacks the expected body-size anchor")
    insert_at = start + position + len(anchor)
...
def main(argv=None) -> int:
    ...
    print("Nginx MCP include contract installed")
    return 0
